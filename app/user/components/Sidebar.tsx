  ...
  return (
    <div className="bg-[#1a1b26] h-screen fixed top-0 left-0 flex flex-col transition-all duration-300 ease-in-out" style={{ width: isMenuOpen ? '12rem' : '4rem' }}>
      ...
      <nav className="flex-grow">
        ...
        <SidebarLink href="/user/dash/generate" icon={<FaHistory className="h-5 w-5" />} text="Generate" isMenuOpen={isMenuOpen} />
        <SidebarLink href="/user/dash/subscription" icon={<FaMoneyBill className="h-5 w-5" />} text="Premium" isMenuOpen={isMenuOpen} />
        <SidebarLink href="/user/dash/models" icon={<FaRobot className="h-5 w-5" />} text="Models" isMenuOpen={isMenuOpen} />
        <SidebarLink href="/user/dash" icon={<FaUser className="h-5 w-5" />} text="Account" isMenuOpen={isMenuOpen} />
      </nav>
      <div className="p-4">
        <WalletBar />
      </div></StarknetProvider>
    </div>
  );
};
